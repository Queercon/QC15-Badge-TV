...
namespace se.nightri.QC15_TV_Badge
{
    public partial class StartFrame : Form
    {
        ...
        public String[] critical = new String[8];
        ...
        {
            string sqlQuery = "SELECT TOP (8) [id0], [badgename], [badges_connected] FROM [badge].[dbo].[badgestats] WHERE [lastseen] >= DATEADD(s, -30, SYSDATETIME()) ORDER By [badges_connected] DESC";
                ...
                using (SqlCommand cmd = new SqlCommand(sqlQuery, con))
                {
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        ...
                        while (reader.Read())
                        {
                            ...
                            critical[i] = (string)reader["badgename"] + prefix + (int)reader["badges_connected"] + " Links";
                            i++;
                        }
                        if (i < 8)
                        {
