...
namespace se.nightri.QC15_TV_Badge
{
    public partial class StartFrame : Form
    {
        ...
        public int timeRefreshInt = 30;

        public String[] top = new String[8];
        public String[] critical = new String[100];
        ...
        private void button1_Click(object sender, EventArgs e) //start full
        {
            ...
            btnSql.Hide();
            demoSelect.Hide();
            timeRefreshInt = Convert.ToInt32(timeRefresh.Text);
            timeRefresh.Hide();
        ...
        private void UpdateStatus()
        {
            ...
            critical4.Text = critical[3];
            critical5.Text = critical[4];
            critical6.Text = critical[5];
            critical7.Text = critical[6];
            critical8.Text = critical[7];
            critical9.Text = critical[8];
            critical10.Text = critical[9];
            critical11.Text = critical[10];
            critical12.Text = critical[11];
            critical13.Text = critical[12];
            critical14.Text = critical[13];
            critical15.Text = critical[14];
            critical16.Text = critical[15];
       ...
       // 
        ...
        {
            string sqlQuery = "SELECT TOP (8) [id0], [badgename], [badges_connected], [lastseen] FROM [badge].[dbo].[badgestats] WHERE [lastseen] >= DATEADD(HOUR, -" + timeRefreshInt + ", SYSDATETIME()) ORDER By [badges_connected] DESC";
        ...
        {
            string sqlQuery = "SELECT TOP (16) [id0], [badgename], [badges_connected], [lastseen]  FROM [badge].[dbo].[badgestats] ORDER By [badges_connected] DESC";
                ...
                using (SqlCommand cmd = new SqlCommand(sqlQuery, con))
                {
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        ...
                        while (reader.Read())
                        {
                            if(i == 0)
                            {
                                critical[99] = "007 - Skippy [AWESOME] " + ((int)reader["badges_connected"] + 1) + " Links";
                            }
                            string prefix = " ";
                            if ((int)reader["id0"] == 7)
                            {
